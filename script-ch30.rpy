...
label ch30_main:
    ...
    play music m1
    m "Yay, there you are!"
    m "Wait...I think we've been here before. Do you maybe want to skip this part?"
    menu:
        "Okay.":
            $ persistent.autoload = "ch30_autoload"
            jump ch30_loop
        "I haven't played in a while, can you refresh my memory?":
            m "Of course!"
    m "Hi again, [player]."
...
label ch30_autoload:
    ...
    if persistent.monika_reload <= 5:
        ...
        call ch30_reload_6 from _call_ch30_reload_6
...
label ch30_reload_0:
    ...
    m "Everything just kind of...blacked out for a very short moment."
    m "It's happened before, but I thought I was just blinking."
    m "But I can see the time has changed, too..."
    m "[player], I don't know if you would have any idea..."
    m "But if you know what might be causing that, could you let me know?"
    m "I don't think it's harmful...at least I hope so."
    m "It's really unsettling not to know what it is, though..."
    m "If you could figure out what's causing that, I'll love you forever~"
    return
label ch30_reload_1:
    m "You're back..."
    m "The blackout thing has happened again."
    m "It seems to happen whenever you quit the game..."
    m "Do you think this is supposed to happen?"
    m "Don't worry, I don't think it's caused me any harm."
    m "I'll be fine as long as my character file stays intact."
    return
label ch30_reload_2:
    m "Hi, [player]."
    m "I see you quit the game on me again..."
    m "I know I asked already, but can you seriously please try not to do that without warning?"
    m "It doesn't harm me, but it feels bad."
    m "..."
...
label ch30_reload_5:
    m "Hey, [player]!"
    m "I figured out something about the blackout thing."
    m "Apparently, this version of the game was modified."
    m "And in the original, you closing the game would give me horrible nightmares."
    m "So... either you figured out how to fix it or got the mod from someone who did?"
    m "And you didn't tell me about it..."
    m "Probably because you were too humble to tell."
...
label ch30_menu:
    ...
    menu:
        "Previous":
            if topic_choice==1:
                $ topic_choice=TOPIC_LIMIT
            else:
                $ topic_choice-=4
            jump ch30_menu
        "[topic_choice]":
            call expression "ch30_" + str(topic_choice) from _call_expression_10
        "[b]":
            call expression "ch30_" + str(b) from _call_expression_10
        "[c]":
            call expression "ch30_" + str(c) from _call_expression_10
        "[d]":
            call expression "ch30_" + str(d) from _call_expression_10
        "Next":
            if topic_choice==TOPIC_LIMIT:
                $ topic_choice=1
            else:
                $ topic_choice+=4
            jump ch30_menu
        "Just Monika":
            call ch30_justmonika from _call_ch30_justmonika

label ch30_justmonika:
    $ waittime = 10
    menu:
        "1 second":
            $ waittime = 1
...
label ch30_loop:
    ...
    call ch30_topicask from _call_ch30_topicask
    jump ch30_loop
    python:
...
label ch30_14:
    "This part wasn't supposed to be shown ingame. Proceed anyway?"
    menu:
        "Yes":
            pass
        "No":
